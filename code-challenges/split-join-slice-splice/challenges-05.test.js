...
const howMuchPencil = (str) => {
  ...
  for (let i=0; i<splitStr.length; i++) {
    sliceStr = splitStr.slice(i, str.length).join('');
...
const splitFoods = (recipe) => {
  ...
  ingr.forEach(ingredient => {
    let splitIngr = ingredient.split(' ');
    splitIngr.splice(0,2);
    result.push(splitIngr.join(' '));
  });
  return result;
};
...
const stepActions = (recipe) => {
  ...
  steps.forEach(step => {
    let splitSteps = step.split(' ');
    let newStep = splitSteps.splice(0,1).join();
    result.push(newStep);
  });
  return result;
};
...
const removeLastCharacters = (str, num) => {
  let splitStr = str.split('');
  return (num>0 ? splitStr.splice(-1, num).join(' ') : str);
};
