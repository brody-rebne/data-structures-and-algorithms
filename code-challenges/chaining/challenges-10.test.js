...
const count = (target, input) => {
  const reducedArr = input.reduce((count, subArr) => {
    const targetCount = subArr.filter(num => num === target).length;
...
const totalSum = (input) => {
  const reducedArr = input.reduce((count, subArr) => {
    const targetCount = subArr.reduce((acc, num) => {
      return acc + num;
    },0);
    return count + targetCount;
  },0);
...
const divisibleByFiveTwoToThePower = (input) => {
  //this is the least legible function name i've ever read
  const newArr = [];
  input.forEach(subArr => {
    const newSubArr = subArr.filter(num => (typeof num === 'number' && num%5===0)).map(num => Math.pow(2, num));
